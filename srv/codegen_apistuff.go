...
func apiCodegenGo() {
	...
	for pkg_name, pkg_dir_path := range pkgsFound {
		...
		buf.WriteString("type apiPkgInfo struct {}\n\n")
...
func apiCodegenTsSdk() {
	...
	apiHandleReflReq(&ApiCtx[Void, apiRefl]{Args: &Void{}, Ret: &api})
	...
	for _, method := range api.Methods {
		apiCodegenTsSdkMethod(&buf, &api, &method)
	}
	for again := true; again; {
		...
		for _, enum_name := range sl.Sorted(Keys(api.Enums)) {
			...
			if apiCodegenTsSdkType(&buf, &api, enum_name, nil, api.Enums[enum_name]) {
				again = true
			}
		}
		for _, struct_name := range sl.Sorted(Keys(api.Types)) {
			if apiCodegenTsSdkType(&buf, &api, struct_name, api.Types[struct_name], nil) {
				again = true
			}
		}
	}
...
func apiCodegenTsSdkMethod(buf *str.Buf, api *apiRefl, method *apiReflMethod) {
	...
	repl := str.Dict{
		...
		"in_type_ident":  apiCodegenTsSdkTypeName(api, method.In),
		"out_type_ident": apiCodegenTsSdkTypeName(api, method.Out),
...
func apiCodegenTsSdkType(buf *str.Buf, api *apiRefl, typeName string, structFields str.Dict, enumMembers []string) bool {
	...
	if typeName == "time.Time" {
		_, _ = buf.WriteString(str.Repl("\nexport type {lhs} = {rhs}",
			str.Dict{"lhs": apiCodegenTsSdkTypeName(api, typeName), "rhs": apiCodegenTsSdkTypeName(api, ".string")}))
	} else if structFields != nil {
		_, _ = buf.WriteString(str.Repl("\nexport type {lhs} = {", str.Dict{"lhs": apiCodegenTsSdkTypeName(api, typeName)}))
		struct_fields := sl.Sorted(Keys(structFields))
		for _, field_name := range struct_fields {
			field_type := structFields[field_name]
			_, _ = buf.WriteString(str.Repl("\n\t{fld}{?}: {tfld}",
				str.Dict{"fld": ToIdent(field_name), "?": If(str.Begins(field_type, "?"), "?", ""), "tfld": apiCodegenTsSdkTypeName(api, field_type)}))
		}
		_, _ = buf.WriteString("\n}\n")
	} else {
		_, _ = buf.WriteString(str.Repl("\nexport type {lhs} = {rhs}\n", str.Dict{
			"lhs": apiCodegenTsSdkTypeName(api, typeName),
...
func apiCodegenTsSdkTypeName(api *apiRefl, typeName string) string {
	for str.Begins(typeName, "?") {
		typeName = typeName[1:]
	}
	...
	if str.Begins(typeName, "[") && str.Ends(typeName, "]") {
		return apiCodegenTsSdkTypeName(api, typeName[1:len(typeName)-1]) + "[]"
	}
	if str.Begins(typeName, "{") && str.Ends(typeName, "}") {
		...
		if _, is_enum := api.Enums[key_part]; is_enum {
			return str.Repl("{ [key in {lhs}]?: {rhs} }", str.Dict{"lhs": apiCodegenTsSdkTypeName(api, key_part), "rhs": apiCodegenTsSdkTypeName(api, val_part)})
		}
		return str.Repl("{ [_:{lhs}]: {rhs} }", str.Dict{"lhs": apiCodegenTsSdkTypeName(api, key_part), "rhs": apiCodegenTsSdkTypeName(api, val_part)})
	}
