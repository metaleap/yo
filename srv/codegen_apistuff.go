...
func codegenTsSdk(apiRefl *apiRefl) (didFsWrites bool) {
	...
	const out_file_path = StaticFilesDirNameYo + "/" + yoSdkTsFileName // app-side path since cur-dir is always app-side
	...
	src_is_changed := codegenForceFull || (len(data) == 0) || (!bytes.Equal(data, src_to_write))
	if src_is_changed {
	...
	WalkDir(StaticFilesDirNameYo, func(path string, dirEntry fs.DirEntry) {
		...
		if (path != out_file_path) && !(IsFile(yo_side_equiv_path) || IsDir(yo_side_equiv_path)) {
			if dirEntry.IsDir() {
				DelDir(path)
			} else {
				DelFile(path)
