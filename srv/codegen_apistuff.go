...
const (
	...
	codegenForceFull          = false // for rare temporary local-dev toggling, usually false
)
...
func codegenTsToJs(inDirPath string, isAppSide bool) {
	const use_tsc = true

	yolog.Println("ts2js in " + inDirPath)
...
func codegenTsSdk(apiRefl *apiRefl) (didFsWrites bool) {
	...
	const out_file_path = StaticFilesDirNameYo + yo_sdk_ts_file_name // app-side path since cur-dir is always app-side
	...
	buf.Write(ReadFile(yo_static_dir_path + yo_sdk_ts_file_name)) // read yo-side version (source-of-truth)
	// emit methods
	...
	if foundModifiedTsFilesYoSide {
		codegenTsToJs(yo_dir_path, false)
		didFsWrites = true
	}
	...
	WalkDir(StaticFilesDirNameYo, func(path string, dirEntry fs.DirEntry) {
		yo_side_equiv_path := yo_dir_path + path
		if !(IsFile(yo_side_equiv_path) || IsDir(yo_side_equiv_path)) {
			if dirEntry.IsDir() {
				DelDir(path)
			} else {
				DelFile(path)
